...
* Edit the file __TransactionsEmulator/src/main/resources/application.properties__, replacing the `geodeUrl` variable with the correct Geode IP. 
* Start pcfdev, if not running yet - `cf dev start`
* Start Greenplum (or PostgreSQL) with `pg_ctl -D /usr/local/var/postgres start` command.
* Run __scripts/create_and_load_postgresql.sh__ which creates a __mydb__ database, uses the DDL statements in __scripts/model_postgres.sql__ to create the database tables, and then imports the data from tables using the csv files __scripts/pos_device.csv__, __scripts/zip_codes_states.csv__ and __scripts/transaction.csv__. 
